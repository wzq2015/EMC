...
REDIPS.table = (function () {
	...
	handler_onmousedown = function (e) {
		...
		if (mouseButton === 1 /*&& td.childNodes.length === 0*/) {
			...
			if (/*td.redips.selected*/rt_selected(td) === true) {
			...
			for (i = 0; i < tr.length; i++) {
				...
				for (j = 0; j < cols; j++) {
					...
					if (mode === 'v') {
						...
						if (c !== undefined && /*c.redips.selected*/rt_selected(c) === true && c.rowSpan > 1) {
							...
							// 设置页面样式
							var formSettings = $('#container').data('formbuilder').options.settings;
							$(n).css('border', formSettings.border + 'px solid')
							.css('border-color', '#' + formSettings.styles.color)
							.css('backgroundColor', '#' + formSettings.styles.backgroundColor);
					...
					// split horizontally
						...
						if (/*c.redips.selected*/rt_selected(c) === true && c.colSpan > 1) {
	...
	row = function (table, mode, index) {
		...
		// delete table row and update rowspan for cells in upper rows if needed
			...
			// 处理从中间删除行的情况
			// 如果当前删除的某个TD未定义，向上找rowSpan>1的，然后-1
			// 如果当前删除的否个TD的rowSpan>1，需要记录下来
			cl = cell_list(table);
			cols = max_cols(table);
			var append_cols;
			// open loop for each cell in last row
			for (i = 0; i < cols; i++) {
				...
				// if cell in last row has rowspan greater then 1
					...
					var _c = {};
					_c.rowSpan = c.rowSpan - 1;
					_c.colSpan = c.colSpan;
			...
			if (append_cols) {
				...
				for (i = 0; i < cols; i++) {
					...
					if (c === undefined && append_cols[i]) {
						...
						nc = table.rows[idx2].insertCell(cellIdx); // 这里的i不妥，需要考虑colSpan,rowSpan
						nc.colSpan = append_cols[i].colSpan;
						nc.rowSpan = append_cols[i].rowSpan;
						cell_init(nc);
						c = nc;
						cl[idx2 + '-' + i] = nc;
					}
