...
<%
	...
	String etc1 =  "onblur=efvalidateObjectNode(this,true,true) maxLength='256' regex='/^[_a-z0-9]{1,256}$/i' nullable='false' minLength='1' maxLength='256' errorPrompt='"+I18nMessages.getText("label.EFSorry","对不起") +"," +I18nMessages.getText("label.ESUserIdRestraint","用户名只能是256位英文字母或者数字字符或下划线。")+"'";
	...
	String etc4 =  "onblur=efvalidateObjectNode(this,true,true) maxLength='16' regex='/^[^\\x22\\x27]+$/' nullable='true' minLength='1' maxLength='16' errorPrompt='"+I18nMessages.getText("label.EFSorry","对不起")+","+I18nMessages.getText("label.ESUserOrgCodeRestraint","组织机构代码应该由至少一个，最多16个字节大小的字符组成，并且不能包含控制字符和单，双引号。")+"'";
	String etc5 =  "onblur=efvalidateObjectNode(this,true,true) maxLength='100' regex='/^[^\\x22\\x27]+$/' nullable='true' minLength='1' maxLength='100' errorPrompt='"+I18nMessages.getText("label.EFSorry","对不起")+","+I18nMessages.getText("label.ESUserOrgNameRestraint","组织机构名称应该由至少一个，最多100个字节大小的字符组成，并且不能包含控制字符和单，双引号。")+"'";
%>
<%@page import="com.baosight.iplat4j.core.resource.I18nMessages"%>
<html>
<head>	
  <script type="text/javascript" src="./EF/iplat-ui-2.0.js"></script>
  <script type="text/javascript" src="./ES/ES26.js"></script>
</head>
 
<body class="bodyBackground">
<form id="ES26" method="POST" action="<%=actionUrl%>" >
		...
		<table>		  
		  <tr>
		    ...
		    <td   width="85%">
			  ...
			  etc="<%=etc1%>"
			  />
			  <span id="inqu_status-0-userId-prompt" color=red style='background-color:white;'>【<%=I18nMessages.getText("label.ESUserIdRestraint","用户名只能是256位英文字母或者数字字符或者下划线。") %>】</span>	<EF:EFButton type="button" ename="checkUser" cname='<%=I18nMessages.getText("label.ESCheckUserIdIsExist","检测用户是否存在") %>' />
		    </td>
		   </tr> 
		   <tr> 
		   ...
		   <tr> 
		    ...
		    <td   width="85%">
			  ...
			  etc="onblur=efvalidateObjectNode(this,true,true) maxLength='24' validateType='phone_with_area_code' "/>
		   ...
		   <tr> 
		    ...
		    【<%=I18nMessages.getText("label.ESUserLockRestraint","默认为不锁定，如果为锁定，则创建的该用户无法登陆系统") %>】
		    </td>	
		   </tr> 	
