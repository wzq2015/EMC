...
FormTopNodeWidget.prototype.bind = function(){
	  ...
	  for(  var key in cols ){
	    ...
	    try{
//	    	if("ID" == col.name)
//	    		$("#("inqu_status-0-PrimaryKeyField").value = row[col.pos]==null?"":row[col.pos];
//	    	else
	    	$("#inqu_status-0-"+col.name)[0].value = row[col.pos]==null?"":row[col.pos];
	    }catch(e){
	...
	if(this._param["IsPrimaryKey"] == "True"){
		 ...
		 var block = __uiDataEiInfo.getBlock("result");
		  ...
		  var cols = block.getBlockMeta().getMetas();  
		    ...
		    var col = cols["ID"];
		    try{
		    	$("#inqu_status-0-"+this._id)[0].value = row[col.pos]==null?"":row[col.pos];
		    	$("#inqu_status-0-"+this._id)[0].readOnly = true;
		    }catch(e){
