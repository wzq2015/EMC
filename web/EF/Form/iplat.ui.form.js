...
//		/*
//		if (isIE) {
//			if (Math.abs(document.body.clientWidth - efform._clientWidth) < efform.RESIZE_SCALE)
//				return;
//			
//			efform._clientWidth = document.body.clientWidth;
//			efform._resizeTime = new Date(); // remember last resize time
//			if (!efform._resizeInterval) // if not exists resize interval,
//				// set it...
//				efform._resizeInterval = setInterval("efform._resize()",
//						efform.RESIZE_INTERVAL);
//			
//		} else {
//			if (Math.abs(document.body.clientWidth - efform._clientWidth) < efform.RESIZE_SCALE)
//				return;
...
efform.createDivWindow = function(div_id, styleClassName, title) {
	...
	ef_subwindow = document.createElement("div");
	ef_subwindow.id = div_id;
	if(!title)
		ef_subwindow.title = getI18nMessages("ef.Select", "请选择");
	else
		ef_subwindow.title = title;
	//ef_subwindow.className = styleClassName;
	//ef_subwindow.style.overflow = "hidden";
	//ef_subwindow.style.display = "none";
	ef_subwindow.innerHTML = "&nbsp;"
	
	document.forms[0].appendChild(ef_subwindow);
 ...
 * @private
 * @param {String}
 *            component_id : 对应的页面DOM对象id，或者对应的Dom对象。
 * @param {String}
 *            treeModel : 树的treeModel。
 * @param {String}
 *            treeDivId : 树的treeDivId。
 * @param {String}
 *            sText : 树的sText。
 * @param {String}
 *            refresh : 弹出框中的内容是否每次弹出时都需要重新刷新（true：刷新）。
 * @return void.
 */
efform.showSubTreeWindow = function(component_id, treeModel, treeDivId, sText,
		refresh, configFunc) {
 ...
 * @return void.
 */
efform.showPopupWindow = function(popupContentId, input_id) {
...
efform.showPopupGridWindow = function(popupContentId, input_id,
		visibleColumnNames, visibleColumnDisplayNames, valueProperty,
		labelProperty, popType, backFillColumnIds, backFillFieldIds) {
...
efform.popGrid_save_onclick = function(gridId, valueProperty, labelProperty,
		popType, backFillColumnIds, backFillFieldIds) {
...
efform.showMultiselectWindow = function(component_id, columnEname) {
	...
	inner_html
			.push("<TR onmouseover='efform.nullfunction();' onclick='efform.nullfunction();' onmousemove='efform.nullfunction();' ><TD colspan=2>");
	var multiSelectOption = "<ul id='gridCellMultiSelectUl' style='margin:0;padding-left:0;border-bottom:1px solid #DDD;height:150px;'>"
			+ labels.join("") + "</ul>";
 ...
 *            eiInfo : 数据;
 * @param {String}
 *            isAllNode: 是否所有div_id下的节点信息，包括三段式、二段式、一段式
