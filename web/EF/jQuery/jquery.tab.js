$.fn.Tab = function(opt){
	...
	var newIframe = function(){	  
		//'marginHeight':'0px','marginWidth':'0px', 'frameBorder':'0px' css 无效
		//移到css中
		/*
		var iframe = $("<iframe />").css({
		    'borderBottomColor':'#999999','borderLeftColor':'#999999',
		    'borderRightColor':'#999999','borderBottomStyle':'solid',
		    'borderLeftStyle':'solid','borderRightStyle':'solid',
		    'borderBottomWidth':'1px','borderLeftWidth':'1px','borderRightWidth':'1px',
		    'borderTop':'none','width':'100%','height':'100%'
		    */
		var iframe = $("<iframe />").addClass("benma_ui_tab_iframe").attr({'scrolling':'no',	'marginHeight':0,'marginWidth':0, 'frameBorder':0});
		return iframe[0];	
	};
	...
	var tabContent=$('<div />').attr('id','jquery_tab_div_content').width(cfg.tabcontentWidth - 2).height("auto").addClass(cfg.tabContentClass);
	...
	.append($("<tr />").
				append($("<td />").addClass(cfg.tabClassInnerLeft).css('background-image','url('+efico.get("eftab.tab1")+')')).
				append($("<td />").addClass(cfg.tabClassInnerMiddle).css('background-image','url('+efico.get("eftab.tab2")+')')
						.append($("<span />").addClass(cfg.tabClassText).addClass('tabTitleText').html('{1}'))).
				append($("<td />").addClass(cfg.tabClassInnerMiddle).css('background-image','url('+efico.get("eftab.tab2")+')')
				        .append($("<div />").addClass(cfg.tabClassClose+' '+cfg.tabClassClose+'_noselected'))).
				append($("<td />").addClass(cfg.tabClassInnerRight).css('background-image','url('+efico.get("eftab.tab2")+')')));
...
//		}
	 ...
	 //item.index, NowNode .NowNode index undefined
	   ...
	   {
		   obj.data('NowNode',cfg.items[item.index-1]);
		   tab.find('li:eq('+(item.index-1)+') td:eq(1)').click();
	   }
