...
EditorUi = function(editor, container)
{
	...
	///////////////////////////////////////////////////////////////////////
				...
				if (graph.isEnabled())
				{
					...
					var isValidDropTarget= (cell!=pool)&&
					(
					(pool&&(target==model.getRoot()))
					||(cell&&(this.isPool(target)||target==model.getRoot()))
					);
...
//							parent == model.getRoot();
   ...
   //graph.container.style.height = '625px';
	...
	// require confirmation when user try to close the window while the graph is modified
	window.onbeforeunload = function(event) {
		if (editor.modified) {
			var workflow = editor.graph.getWorkflow();
			var name = workflow.getAttribute('Name').trim();
			var displayName = workflow.getAttribute('DisplayName').trim();
			return '流程"' + (displayName ? displayName : name) + '"尚未保存';
		}
	};
...
EditorUi.prototype.save = function()
{
	...
	parent.dialog('保存工作流', 'EW0121', 
			{'width':300,'height':300,'params':'efHeadTail=none'});
};
...
EditorUi.prototype.createUi = function()
{
	...
	//$(this.vsplit).splitter({splitVertical:true,A:$(this.sidebarContainer),B:$(this.diagramContainer),closeableto:0});
...
EditorUi.prototype.showOutline = function() {
	if (!this.outline) {
		...
		win.addListener(mxEvent.RESIZE_END, function(){outline.update()});
		this.outline = win;
