...
Actions.prototype.init = function()
{
	...
	this.addAction('viewXml', function() {
		parent.dialog('查看/编辑工作流XML', 'EW0105');
	});
	this.addAction('settings', function() {
		var options={'width':550,'height':450,'params':'efHeadTail=none'};
		parent.dialog('流程属性设置', 'EW0102',options);
	});
	this.addAction('dataFields', function() {
		var options={'width':550,'height':480,'params':'efHeadTail=none'};
		parent.dialog('设置相关数据', 'EW0103',options);
	});
	this.addAction('eagleye', function() { 
		ui.toggleOutline();
	});
	this.addAction('attributeSetting', function() {
		var graph=editor.graph;
		...
		var et = cell.getValue();
		var nodeName=et.nodeName.toLowerCase();
		if(nodeName == "start" ) {
			var options={'width':650,'height':530,'params':'efHeadTail=none'};
		    parent.dialog('开始活动信息设置', 'EW0101',options);
		    parent.$("#EW0101").css('height','530px');
		}
		if(nodeName == "manual" ) {
			var options={'width':650,'height':530,'params':'efHeadTail=none'};
		    parent.dialog('人工活动信息设置', 'EW0101',options);
		    parent.$("#EW0101").css('height','530px');			
		}
		if(nodeName == "subprocess") {	
			var options={'width':650,'height':530,'params':'efHeadTail=none'};
		    parent.dialog('子流程活动信息设置', 'EW0104',options);
		    parent.$("#EW0104").css('height','530px');		
		}
		if(nodeName == "countersignature") {			
			var options={'width':650,'height':530,'params':'efHeadTail=none'};
		    parent.dialog('会签活动信息设置', 'EW0106',options);
		   parent.$("#EW0106").css('height','530px');
		}
		if(nodeName == "code") {			
			var options={'width':650,'height':530,'params':'efHeadTail=none'};
		    parent.dialog('可编程活动信息设置', 'EW0107',options);
		    parent.$("#EW0107").css('height','530px');			
		}
		if(nodeName == "fork") {			
			var options={'width':500,'height':350,'params':'efHeadTail=none'};
		    parent.dialog('并发活动信息设置', 'EW0108',options);
		   //parent.$("#EW0108").css('height','350px');			
		}
		if(nodeName == "join") {	
			var options={'width':500,'height':350,'params':'efHeadTail=none'};
		    parent.dialog('聚合活动信息设置', 'EW0109',options);
		   //parent.$("#EW0109").css('height','350px');		    
		}
		if(nodeName == "end") {	
			var options={'width':500,'height':350,'params':'efHeadTail=none'};
		    parent.dialog('结束活动信息设置', 'EW0110',options);
		   //parent.$("#EW0110").css('height','350px');			
		}
		if(nodeName == "transition") {
			var options={'width':500,'height':350,'params':'efHeadTail=none'};
		    parent.dialog('转移信息设置', 'EW0111',options);
		   //parent.$("#EW0111").css('height','350px');
		}
	});
	// Edit actions
	...
	// Arrange actions
	this.addAction('toFront', function() { graph.orderCells(true); }, null, null, 'Ctrl+F');
	this.addAction('toBack', function() { graph.orderCells(false); }, null, null, 'Ctrl+B');
