...
//	this.addUmlPalette(false);
	this.addBasicPalette(dir, false);
//	this.addTransitionPalette(true);
...
//		';fillColor=#ffffff;strokeColor=#000000;strokeWidth=2');
//	this.addWorkflowPalette('clipart', '图元', dir + '/workflow/', '.png',
//		['start', 'end', 'manual', 'fork','join','subprocess','code','countersignature']);
...
Sidebar.prototype.addWorkflowPalette = function(id, title, prefix, postfix, items)
{
	this.addPalette(id, title, true, mxUtils.bind(this, function(content)
    {
    	for (var i = 0; i < items.length; i++)
		{
			...
			content.appendChild(this.createVertexTemplate('image;image=' + icon, 50, 50, new WfModel().getWorkflowTokenObject(items[i])));
		}
    }));
};
...
Sidebar.prototype.humanImage = STENCIL_PATH + '/clipart/Suit1_128x128.png';
Sidebar.prototype.subprocessImage = STENCIL_PATH + '/clipart/SubProcess.png';
Sidebar.prototype.colorStyle=";strokeWidth=2;strokeColor=#99CCFF;fillColor=#CCE5FF;gradientColor=#FFFFFF;";
Sidebar.prototype.joinColorStyle=";strokeWidth=2;strokeColor=#62A92B;fillColor=#F4F5D6;gradientColor=#FFFFFF;";
Sidebar.prototype.endColorStyle=";strokeWidth=2;strokeColor=#8A151A;fillColor=#8A151A;gradientColor=#FFFFFF;";
Sidebar.prototype.innerCellColorStyle=";strokeColor=#ffffff;fillColor=#27aa2d;gradientColor=#65e96e;";
...
{
	this.addStencilPalette('bpmn', '图元', dir + '/workflow.xml',
...
Sidebar.prototype.addBasicPalette = function(dir, expand) {
	this.addStencilPalette('bpmn', '图元', dir + '/workflow.xml',
			this.colorStyle, [ 'Cancel', 'Error', 'Link', 'Message',
					'Compensation', 'Multiple', 'Rule', 'Timer' ], function(
					content) {							
			
				//可编程活动
			
			var classCell1 = new mxCell('', new mxGeometry(0, 0, 20, 14),
					'shape=tape;verticalLabelPosition=bottom;verticalAlign=top' + this.innerCellColorStyle);
			classCell1.vertex = true;
			classCell1.connectable = false;
			
			var classCell =this.editorUi.editor.nodetypes["Code"].createCell(new WfModel()
					.getWorkflowTokenObject('code'),0,0,100,50,"");

			content.appendChild(this.createVertexTemplateFromCellsAndSBCells(
					[ classCell ],[classCell1], 100, 50));				
					
			// 会签活动		
					
			var classCell1 = new mxCell('', new mxGeometry(0, 0, 20, 14),
					'shape=step;verticalLabelPosition=bottom;verticalAlign=top' + this.innerCellColorStyle);
			classCell1.vertex = true;
			classCell1.connectable = false;
			
            var classCell =this.editorUi.editor.nodetypes["CounterSignature"].createCell(new WfModel()
					.getWorkflowTokenObject('countersignature'),0,0,100,50,"");
					
			content.appendChild(this.createVertexTemplateFromCellsAndSBCells(
					[ classCell ],[classCell1], 100, 50));
			// 人工活动
			
			var classCell1 = new mxCell('', new mxGeometry(0, 0, 14, 14),
					'shape=actor;verticalLabelPosition=bottom;verticalAlign=top' + this.innerCellColorStyle);
			classCell1.vertex = true;
			classCell1.connectable = false;
			
			 var classCell =this.editorUi.editor.nodetypes["Manual"].createCell(new WfModel()
					.getWorkflowTokenObject('manual'),0,0,100,50,"");

			content.appendChild(this.createVertexTemplateFromCellsAndSBCells(
					[ classCell ],[classCell1], 100, 50));
			// 子流程活动			
			var classCell1 = new mxCell('', new mxGeometry(0.5, 1, 14, 14),
					'shape=plus' + this.innerCellColorStyle);
			classCell1.vertex = true;
			classCell1.connectable = false;
             var classCell =this.editorUi.editor.nodetypes["SubProcess"].createCell(new WfModel()
					.getWorkflowTokenObject('subprocess'),0,0,100,50,"");
			content.appendChild(this.createVertexTemplateFromCellsAndSBCells(
					[ classCell ],[classCell1], 100, 50));
			//泳道
			var classCell1 = new mxCell('', new mxGeometry(0, 0, 80, 40),
					'swimlane;horizontal=0;strokeColor=#99CCFF;');			
			classCell1.vertex = true;
			classCell1.connectable = false;
			
			var classCell =this.editorUi.editor.nodetypes["Swimlane"].createCell(new WfModel()
					.getWorkflowTokenObject('swimlane'),0,0,910,120,"");
...
Sidebar.prototype.addTransitionPalette = function(expand)
{
	this.addPalette('general', mxResources.get('general'), expand || true, mxUtils.bind(this, function(content)
	{
//		content.appendChild(this.createVertexTemplate('swimlane', 200, 200, 'rectangle'));
...
Sidebar.prototype.createDropHandler = function(cells, allowSplit)
{
	return function(graph, evt, target, x, y)
	{
		...
		if (cells.length > 0)
		{
			...
			if(target != null) 
				validDropTarget=graph.isValidDropTarget(target, cells, evt);
 ...
 * Creates a drop handler for inserting the given cells.
 */
Sidebar.prototype.createVertexTemplateFromCellsAndSBCells = function(cells,sbCells, width, height)
{

	var elt = this.createItem(sbCells);
...
{
//	var classCell1 = new mxCell('', new mxGeometry(0, 0, 20, 14),
//					'shape=tape' + this.innerCellColorStyle);
//			classCell1.vertex = true;
//			classCell1.connectable = false;
//			classCell1.geometry.relative = true;
//			classCell1.geometry.offset = new mxPoint(5, 5);
//	var tempCells=[classCell1];
//	var elt = this.createItem(tempCells);
...
Sidebar.prototype.createEdgeTemplate = function(style, width, height, value)
{
	...
	var cells = [new mxCell(new WfModel().getWorkflowTransitionObject(), new mxGeometry(0, 0, width, height), style)];
	...
	this.addPalette(id, title, true, mxUtils.bind(this, function(content)
    {
		...
		mxStencilRegistry.loadStencilSet(stencilFile, mxUtils.bind(this, function(packageName, stencilName, displayName, w, h)
		{
			if (ignore == null || mxUtils.indexOf(ignore, stencilName) < 0)
			{
				...
				var classCell =this.editorUi.editor.nodetypes[stencilName].createCell(new WfModel()
					.getWorkflowTokenObject(stencilName.toLowerCase()),0,0,Math.round(w * scale), Math.round(h * scale),"");
			var colorStyle=null;
			if(stencilName.toLowerCase()=="end")colorStyle=this.endColorStyle;
			else colorStyle=this.joinColorStyle;
			var classCell1 = new mxCell('', new mxGeometry(0, 0, Math.round(w * scale), Math.round(h * scale)),
					'shape='+ packageName + stencilName.toLowerCase()+';verticalLabelPosition=bottom;verticalAlign=top;' + colorStyle);
			classCell1.vertex = true;
			classCell1.connectable = false;

			   content.appendChild(this.createVertexTemplateFromCellsAndSBCells(
					[ classCell ],[classCell1], Math.round(w * scale), Math.round(h * scale)));
					
//				content.appendChild(this.createVertexTemplate('shape=' + packageName + stencilName.toLowerCase() + style,
//					Math.round(w * scale), Math.round(h * scale), new WfModel().getWorkflowTokenObject(stencilName.toLowerCase())));
			}
		}), true);
