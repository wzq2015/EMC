/**
 * $Id: Graph.js,v 1.45 2012-08-02 13:56:37 gaudenz Exp $ Copyright (c)
...
Graph = function(container, model, renderHint, stylesheet) {
	...
	// 'edgeStyle=elbowEdgeStyle;elbow=horizontal;strokeColor=#007FFF';
	this.configure();
	if (stylesheet == null) {
		this.loadStylesheet();
	}
	...
	this.panningHandler.addListener(mxEvent.PAN_START, mxUtils.bind(this,
					function() {
						this.container.style.cursor = 'pointer';
					}));

	this.panningHandler.addListener(mxEvent.PAN_END, mxUtils.bind(this,
					function() {
	...
	this.connectionHandler.tapAndHold = function(me, state) {
		...
		} else if (this.graph.isCellSelected(state.cell)
...
Graph.prototype.currentEditingCell = null;
...
Graph.prototype.configure = function() {
	var node = mxUtils.load(CONFIG_PATH + '/wfgraph.xml').getDocumentElement();
	var dec = new mxCodec(node.ownerDocument);
	dec.decode(node, this);
};


Graph.prototype.selectVertices = function(c) {
...
//								&& this.model.getChildCount(c) == 0
						&&c.value&&(c.value.tagName=="Start"||
						c.value.tagName=="End"||
						c.value.tagName=="Manual"||
						c.value.tagName=="Fork"||
						c.value.tagName=="Join"||
						c.value.tagName=="SubProcess"||
						c.value.tagName=="Code"||
						c.value.tagName=="CounterSignature")
...
Graph.prototype.getTooltipForCell = function(cell) {
	...
	// var displayName=setting.getAttribute('DisplayName');
	// tip+='活动名称:'+name+"\n";
	// tip+='显示名称:'+displayName+"\n";
	// }
...
Graph.prototype.convertValueToString = function(cell) {
	if (cell.value != null && typeof(cell.value) == 'object') {
		return cell.value.getAttribute('DisplayName');
	}
...
Graph.prototype.cellLabelChanged = function(cell, value, autoSize) {
	if (cell.value != null && typeof(cell.value) == 'object') {
		...
		tmp.setAttribute('DisplayName', value);
		value = tmp;
	}
	...
	this.fireMouseEvent = function(evtName, me, sender) {
		if (evtName == mxEvent.MOUSE_DOWN) {
			if (!this.panningHandler.isMenuShowing()) {
				menuCell = me.getCell();
				showMenu = (menuCell != null)
						? this.isCellSelected(menuCell)
			...
			if (showMenu && !this.isEditing()) {
				if (!this.panningHandler.isMenuShowing()) {
					...
					this.panningHandler.popup(x + 16, y, menuCell, me
									.getEvent());
				}
...
Graph.prototype.insertEdge = function(a, b, c, d, e, f) {
	var transObj = new WfModel().getWorkflowTransitionObject();
	transObj.setAttribute('name', c || "");
	return this.addEdge(this.createEdge(a, b, transObj, d, e, f), a, d, e)
};

Graph.prototype.getUrl = function() {
	var url = location.href;
	var lastIndex = url.lastIndexOf("/");
	return url.substring(0, lastIndex).replace('EW/WorkflowDesigner', '');
};

Graph.prototype.travelCells = function(cellHandler, includeNonBizNode) {
	var q = [this.getDefaultParent()];

	while (q.length) {
		var cell = q.pop();
		var isBizNode = cell.value && cell.value.nodeName
				&& cell.value.getAttribute('IsBizNode') == 'true';
		if ((isBizNode || includeNonBizNode) && cellHandler(cell) === false)
			return;
		if (isBizNode) // bizNode does not have sub-cells
			continue;
		for (var i = cell.getChildCount() - 1; i >= 0; i--)
			q.push(cell.getChildAt(i));
	}
}

Graph.prototype.getActivities = function() {
	var activities = [];
	this.travelCells(function(cell) {
				if (cell.vertex)
...
Graph.prototype.getTransitions = function() {
	var transitions = [];
	this.travelCells(function(cell) {
				if (cell.edge)
...
Graph.prototype.getWorkflow = function() {
	return this.getModel().getCell('0').value;
};
...
(function() {
	...
	if (touchStyle) {
		...
		if (mxClient.IS_TOUCH) {
			...
			mxPanningHandler.prototype.isPanningTrigger = function(me) {
				...
				return (this.useLeftButtonForPanning && (this.ignoreCell || me
						.getState() == null)/*
											 * && mxEvent.isLeftMouseButton(evt)
											 */)
						|| (mxEvent.isControlDown(evt) && mxEvent
								.isShiftDown(evt))
		...
		mxGraphHandler.prototype.mouseDown = function(sender, me) {
			...
			if (this.graph.isCellSelected(me.getCell())
					&& this.graph.getSelectionCount() > 1) {
		...
		mxVertexHandler.prototype.init = function() {
			...
			if (showConnectorImg && this.graph.connectionHandler.isEnabled()
				...
				if (!mxClient.IS_TOUCH) {
					this.connectorImg.setAttribute('title', mxResources
									.get('connect'));
					mxEvent.redirectMouseEvents(this.connectorImg, this.graph,
							this.state);
				}
		...
		mxVertexHandler.prototype.redrawTools = function() {
			if (this.state != null && this.connectorImg != null) {
				...
				if (mxVertexHandler.prototype.singleSizer) {
					this.connectorImg.style.left = (this.state.x
							+ this.state.width - this.connectorImg.offsetWidth
							/ 2)
							+ 'px';
					this.connectorImg.style.top = (this.state.y - this.connectorImg.offsetHeight
							/ 2)
							+ 'px';
				} else {
					this.connectorImg.style.left = (this.state.x
							+ this.state.width + mxConstants.HANDLE_SIZE / 2 + 4/*
																				 * - 2
																				 * padding
																				 */)
							+ 'px';
					this.connectorImg.style.top = (this.state.y + (this.state.height - this.connectorImg.offsetHeight)
							/ 2)
							+ 'px';
				}
			}
	...
	{
		mxConnectionHandler.prototype.connectImage = new mxImage(IMAGE_PATH
						+ '/connector.png', 15, 15);
	}
})();
