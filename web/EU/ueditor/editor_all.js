...
//                img.style.cssText += ';margin:' + (first.margin||0) +"px;" + 'border:' + (first.border||0) +"px solid #000";
            ...
            ci = opt[0];
            if(opt.length == 1){
                str = '<img id="articleImg" src="'+ci.src+'" '+ (ci.data_ue_src ? ' data_ue_src="' + ci.data_ue_src +'" ':'') +
    ...
    UE.plugins['source'] = function (){
        ...
        me.commands['source'] = {
            execCommand: function (){
                ...
                if (sourceMode) {
                	//alert("1");
                    //alert("content1234:"+me.getContent());
                    me.undoManger && me.undoManger.save();
...
UE.plugins['video'] = function (){
     ...
     */
     function creatInsertStr(url,width,height,align,toEmbed,addParagraph){
     	var info = new EiInfo();
        EiCommunicator.send( "ECAM99", "queryLocal" , info, null);
        var furl=ajaxEi.get("bashPath")+"/EC/File/video/vcastr22.swf";//服务器路径
        //var furl1 = ajaxEi.get("uploadPath")+"EC/File/video/vcastr22.swf";//本地路径
 		
	    var info = new EiInfo();
	    //info.set("realUrl",furl1);
	    EiCommunicator.send( "ECAM99", "copy" , info, null ); 
	    if(ajaxEi!=null){	
        ...
        return  ' <object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" '+
	            ...
	            'height="' + height +'" width="' + width + ' align="' + align + '"'+
				( align !="none" ? ' style= "'+ ( align == "center"? "display:block;":" float: "+ align )  + '"' :'' ) +
	             '"> '+
	            '<param name="movie" '+
				'value="'+furl+'?vcastr_file=' + url + '"> '+
				'<param name="quality" value="high"> '+
				'<param name="allowFullScreen" value="true" /> '+
				'<embed '+
				'src="'+furl+'?vcastr_file=' + url + '" '+
				'quality="high" '+
...
(function (){
    ...
    Menu.prototype = {
        ...
        _Popup_HtmlTpl: Popup.prototype.HtmlTpl,
        HtmlTpl: function (){
            if (this.items.length == 0) {
                return this._Popup_HtmlTpl();
            }
            var buff = [];
            for (var i=0; i<this.items.length; i++) {
                var item = this.items[i];
...
(function (){
    ...
    var  iframeUrlMap ={
        ...
        'insertimage':'./EC/DM/ECDM0105.jsp',
        ...
        /*'insertvideo':'?efFormEname=ECDMTT&type=3',*/
        'insertvideo':'./EC/DM/ECDM0106.jsp',
        'help':'~/dialogs/help/help.html',
        'highlightcode':'~/dialogs/code/code.html',
        'emotion':'~/dialogs/emotion/emotion.html',
        'wordimage':'~/dialogs/wordimage/wordimage.html',
        'attachment':'./EC/DM/ECDM0107.jsp',
        /*'attachment':'~/dialogs/attachment/attachment.html',*/
        /*'attachment':'?efFormEname=ECDMTT&type=0',*/
        'insertframe':'~/dialogs/insertframe/insertframe.html',
        'edittd':'~/dialogs/table/edittd.html',
        'webapp':'./EC/DM/ECDM0108.jsp',
        /*'webapp':'~/dialogs/webapp/webapp.html',*/
        /*'webapp':'?efFormEname=ECDMTT&type=2',*/
                    ...
                    editorui[cmd] =function (editor, iframeUrl, title){
                        ...
                        var ui = new editorui.Button({
                            ...
                            onclick: function (){
                                ...
                                EiCommunicator.send( "ECAM99", "queryLocal" , info, null);
                                var bashPath=ajaxEi.get("bashPath");
                                var siteAlias = ajaxEi.get("siteAlias");  //站点别名
                                ...
                                if(cmd=="wordimage"){//wordimage需要先判断是否存在word_img属性再确定是否打开
                                ...
                                }else if(cmd=="insertimage"){  //插入图片
                                	var url = iframeUrlMap["insertimage"];
									url += "?nodeId="+nodeId+"&nodeType="+nodeType;
								    //alert("url:"+url);
                                	//点击图片图标弹出图片上传框<模式化窗口> return:filePath;         
                                	...
                                	if(typeof(fileId) != "undefined"){
	                                	...
	                                	var fid = fileId.substring(index1+1,index2);
	                                		//alert("fid:"+fid);
	                                	if(window.parent.efform_onPopupReturnM != undefined)
	                                			window.parent.efform_onPopupReturnM(fid);  //将fileId返回到ECAM02界面
	                                		imgUrl =  fileId;//本地图片路径
	                                	//}	  
	                                	//调用编辑器方法插入图片的有效url
                                ...
                                }else if(cmd=="insertvideo"){  //插入视频
                                	
                                	var url = iframeUrlMap["insertvideo"];
									url += "?nodeId="+nodeId+"&nodeType="+nodeType;
                                	//点击视频图标弹出视频选择框<模式化窗口> return:filePath;         
                                	var path = window.showModalDialog(url,null,"dialogWidth:350px;dialogHeight:250px;help:no;status:no");   
                                	
          							if(typeof(path) != "undefined"){
          								var filePath = bashPath+path; 
                                ...
                                }else if(cmd=="attachment"){    //插入附件
                                	var url = iframeUrlMap["attachment"];
                                	url += "?nodeId="+nodeId+"&nodeType="+nodeType;

                                	//点击附件图标弹出视频选择框<模式化窗口> return:filePath;         
                                	var str = window.showModalDialog(url,null,"dialogWidth:350px;dialogHeight:250px;help:no;status:no");   
          							...
          							if(typeof(str) != "undefined"){
                                		...
                                		var fid = str.substring(index1+1,index2);
	          							var attUrl = "";
	          							
	          							if(window.parent.efform_onPopupReturnM != undefined)
	          								window.parent.efform_onPopupReturnM(fid);
	          							attUrl= path; 
	                                	var local = "<a href='"+attUrl+"'>"+fileName+"</a>"
	                                	//调用编辑器方法插入图片的有效url
	                                	editor.execCommand("insertHTML",local);
	                                }
                                }
                                else if(cmd=="webapp"){        //插入flash
                                	var url = iframeUrlMap["webapp"];
									url += "?nodeId="+nodeId+"&nodeType="+nodeType;	
                                	//点击flash图标弹出视频选择框<模式化窗口> return:filePath;         
                                	var path = window.showModalDialog(url,null,"dialogWidth:350px;dialogHeight:250px;help:no;status:no");   
          							if(typeof(path) != "undefined"){
	          							var filePath = bashPath+path; 
	                                	
	                                	//调用编辑器方法插入视频(flash)的有效url
	                                	editor.execCommand('insertvideo', {
								            url: filePath,
								            width: '360px',
								            height: '240px',
