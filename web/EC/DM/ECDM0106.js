 ...
 function upload(){
	...
	document.forms[0].action += "&fileDes="+encodeURI(fileDes);
   	...
   	if(check()){
   	//防止上传出错再次向iframe中提交表单时，表单提交到新窗口
   		document.getElementById("uploadFileIframe").contentWindow.name = "uploadFileIframe";
...
function checkSuffix(){
   ...
   //是否符合视频类型的文件的后缀名   
   //视频:AVI, .wma, .rmvb,.rm, .flash,.mp4,.mid, 3GP 
    ...
    efform_onPopupReturn1 = function(winId, obj){
		window.returnValue=obj;
	   	window.close();
   }

  //上传出错，显示提示信息
    on_upload_fail = function(msg){
 	   alert(msg);
    }
    //上传成功，显示iframe页面
