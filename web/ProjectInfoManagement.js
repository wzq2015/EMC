...
function onTreeClick(node) {
	...
	if (node.attributes.type == 'project') {
		...
		EiCommunicator.send("CMIMDeviceGroup", "queryDeviceGroupById", info, queryDeviceGroupCallback);
	}
	else if (node.attributes.type == 'device') {
		info.set("f_deviceId", node.attributes.logicid);
		EiCommunicator.send("CMIMDevice", "queryDeviceById", info, queryDeviceCallback);
	}
...
queryEmcprojectCallback = {
	onSuccess : function(eiInfo) {
		...
		$('#txtConstructDate').val(row[metas.f_emcprojectConstructdate.pos]);
		$('#txtCommissionDate').val(row[metas.f_emcprojectCommissioningdate.pos]);
		$('#txtProjectRes').val(row[metas.f_emcprojectResponsible.pos]);
		$('#txtConstructUnit').val(row[metas.f_emcprojectConstructunit.pos]);
		$('#txtStartDate').val(row[metas.f_emcprojectStartdate.pos]);
		$('#txtEndDate').val(row[metas.f_emcprojectEnddate.pos]);
