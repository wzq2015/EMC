var toolbar = 
	[
		...
		{ text:'编辑计算过程', iconCls:'icon-save', handler:editCalcstep }
	];
...
function saverow() {
	...
	info.set("f_energysavingTypeTargetvalue", row.f_energysavingTypeTargetvalue);

	if (actionType == ActionType.Adding) {
		EiCommunicator.send("CMIMEnergysavingType", "insertEnergysavingType", info, savecallback);
	}
	else if (actionType == ActionType.Editing) {
		info.set("f_energysavingTypeId", row.f_energysavingTypeId);
		EiCommunicator.send("CMIMEnergysavingType", "updateEnergysavingType", info, savecallback);
	}
}
...
function deleterow(index) {
	...
	$.messager.confirm('确认', '是否确认删除节能量类型？', 
		function(r) {
			if (r) {
				var row = $('#datagrid').datagrid('getRows')[index];
				var info = new EiInfo();
				info.set("f_energysavingTypeId", row.f_energysavingTypeId);
				EiCommunicator.send("CMIMEnergysavingType", "deleteEnergysavingType", info, deletecallback);
			}
		});
}
...
function saveFormula() {
	...
	if (row) {
		...
		if (isFormula) {
			...
			EiCommunicator.send("CMIMEnergysavingType", "updateFormula", info, saveFormulaCallback);
		}
		else {
			info.set("f_energysavingTypeId", row.f_energysavingTypeId);
...
saveFormulaCallback = {
	onSuccess : function(eiInfo){
		if (eiInfo.get("errorcode") == "-1") {
			$.messager.alert('错误','保存计算公式或计算过程失败');
			return;
		}
