	...
	if( grid_id == "ef_grid_result" ){
		var grid = efgrid.getGridObject("ef_grid_result");
      	var columnEname = grid.getColumn(col_index, TYPE_DATA).getEname();
       	//var value = grid.getCellValue(row_index,1,TYPE_DATA);
       	var rowData = grid.getRowData(row_index);
       	value = rowData.layoutId;
       	ef.get("son_inqu_status-0-layoutId").value = value;
       	efgrid.submitInqu( "ef_grid_son", "EVCM", "EVCM07","queryRow");
	}
...
//添加[布局列记录] 点击新增按钮的时候触发函数
button_insertson_onclick = function () {
	var layoutid = ef.get("son_inqu_status-0-layoutId").value;
	if(!layoutid){
		alert("请选择布局！");
		return;
	}
     //过滤特殊字符
	var grid = efgrid.getGridObject("ef_grid_son");
	var rows = grid.getCheckedRows();
	var vFlag = vlidateAddNodeId(grid,rows,"布局列名称");
 
	if(!vFlag){
		 return;
	  }
	
	//验证[列类型-百分比选项][列数值]取值范围1-100
	var colFlag = getColumnVa(grid,rows);
	if(!colFlag){
		return;
	}
	
	efgrid.submitForm( "ef_grid_son", "EVCM","EVCM07","insertRow",true);
}
...
//点击[布局列记录] 修改按钮
button_updateson_onclick = function () {
       //过滤特殊字符
	var grid = efgrid.getGridObject("ef_grid_son");
	var rows = grid.getCheckedRows();
	var vFlag = vlidateAddNodeId(grid,rows,"布局列名称");
	...
	efgrid.submitForm( "ef_grid_son", "EVCM","EVCM07","updateRow",true);
}
//----------------------------------------------添加布局数据验证 end------------------------------------
...
button_delete_onclick = function () {
	...
	var son_grid = efgrid.getGridObject( "ef_grid_son" );
	son_grid.refresh(ajaxEi);
}


//点击删除按钮
button_deleteson_onclick = function () {
	efgrid.submitForm( "ef_grid_son", "EVCM","EVCM07","deleteRow", true );
}
