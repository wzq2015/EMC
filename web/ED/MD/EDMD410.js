/**
 * Generate time : 2011-11-16 11:11:59
 */
...
var tabIndexOnDblClick = 3;	//双击记录时，切换到的Tab页(0是第一个页面, <0表示不切换)
...
composePath = function(frame, path, index) {
	...
	case 0: //第1个Tab页
		return path;
	case 1: //第2个Tab页
		if (selected["modelType"] == "1") {
//			alert(selected["id"]);
			return path + "&methodName=query&inqu_status-0-modelId=" + selected["id"];
		} else {
//			alert(selected["dataModelId"]);
			return path + "&methodName=query&inqu_status-0-modelId=" + selected["dataModelId"];
		}
	case 2: //第3个Tab页
		if (selected["modelType"] == "0") {
//			alert(selected["id"]);
			return path + "&methodName=query&inqu_status-0-modelId=" + selected["id"];
		} else {
//			alert(selected["condModelId"]);
			return path + "&methodName=query&inqu_status-0-modelId=" + selected["condModelId"];
		}
	case 3: //第4个Tab页
		return path + "&methodName=query&inqu_status-0-modelId=" + selected["id"];
	case 4: //第5个Tab页
		return path + "&methodName=queryByModel&inqu_status-0-modelId=" + selected["id"];
	case 5: //第6个Tab页
